/**
 * API Usage Monitor - Qwen & Gemini 전용
 * Claude는 Claude Code 무제한 사용으로 제외
 */
...
class APIUsageMonitor extends EventEmitter {
    constructor() {
        ...
        // API 요금 정보 (USD per 1K tokens) - 실제 사용하는 API만
        this.pricing = {
            qwen: {
        ...
        // Claude Code 사용 현황 (비용 없음, 정보만)
        this.claudeCodeUsage = {
            sessions: 0,
            totalInteractions: 0,
            filesProcessed: 0,
            codeGenerated: 0,
            note: 'Claude Max x20 구독으로 무제한 사용'
        };
    ...
    async initialize() {
        console.log('🚀 API Usage Monitor 초기화 중 (Qwen & Gemini)...');
        ...
        console.log('📌 Claude는 Claude Code 무제한 사용으로 비용 추적 제외');
    }
    ...
    /**
     * Claude Code 사용 추적 (정보용, 비용 없음)
     */
    trackClaudeCodeUsage(type = 'interaction') {
        this.claudeCodeUsage.totalInteractions++;
        
        switch(type) {
            case 'file':
                this.claudeCodeUsage.filesProcessed++;
                break;
            case 'code':
                this.claudeCodeUsage.codeGenerated++;
                break;
            case 'session':
                this.claudeCodeUsage.sessions++;
                break;
        }
    ...
    getSummary() {
        ...
        // Claude Code 정보 추가
        summary.claudeCode = this.claudeCodeUsage;
    ...
    startWebSocketServer() {
        this.wss = new WebSocketServer({ port: 8105 });
        ...
        console.log('🌐 WebSocket 서버 시작 (포트 8105)');
    }
    ...
    async saveData() {
        ...
        const data = {
            timestamp: new Date(),
            usage: this.usage,
            claudeCodeUsage: this.claudeCodeUsage,
    ...
    async loadPreviousData() {
        ...
        try {
            ...
            if (parsed.claudeCodeUsage) {
                Object.assign(this.claudeCodeUsage, parsed.claudeCodeUsage);
            }
...
<head>
    ...
    <title>API Usage Monitor - Qwen & Gemini</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="claude-info">
            <h2>Claude Code Status <span class="free-badge">무제한</span></h2>
            <p>Claude Max x20 구독으로 무제한 사용 중</p>
            <div style="margin-top: 10px;">
                세션: <span id="claude-sessions">0</span> | 
                상호작용: <span id="claude-interactions">0</span> | 
                파일 처리: <span id="claude-files">0</span> | 
                코드 생성: <span id="claude-code">0</span>
            </div>
        </div>
        
        <div class="total-cost">
            💰 API 비용 (Qwen + Gemini): <span id="totalCost">$0.00</span> (<span id="totalCostKRW">₩0</span>)
        </div>
    ...
    <script>
        const ws = new WebSocket('ws://localhost:8105');
        ...
        function updateDashboard(data) {
            ...
            // Claude Code 정보 업데이트
            if (data.claudeCode) {
                document.getElementById('claude-sessions').textContent = data.claudeCode.sessions;
                document.getElementById('claude-interactions').textContent = data.claudeCode.totalInteractions;
                document.getElementById('claude-files').textContent = data.claudeCode.filesProcessed;
                document.getElementById('claude-code').textContent = data.claudeCode.codeGenerated;
            }
            
            // 각 프로바이더별 업데이트
            ['qwen', 'gemini'].forEach(provider => {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    // 실제 사용 예시
    console.log('\n📊 API Usage Monitor 실행 중\n');
    console.log('💡 Claude는 Claude Code 무제한 사용으로 비용 추적 제외');
    console.log('💡 Qwen과 Gemini API만 비용 추적\n');
    
    // Claude Code 사용 추적 (비용 없음)
    monitor.trackClaudeCodeUsage('session');
    
    // 대시보드 생성
    await monitor.generateDashboard();
    console.log('\n🌐 대시보드: http://localhost:8105');
    ...
    setInterval(() => {
        ...
        console.log('📊 API 사용량 모니터링 (실제 비용 발생 API만)');
        console.log('='.repeat(60));
        
        console.log('\n✅ Claude Code (무제한 사용):');
        console.log(`  세션: ${summary.claudeCode.sessions}`);
        console.log(`  상호작용: ${summary.claudeCode.totalInteractions}`);
        console.log(`  ${summary.claudeCode.note}`);
        
        for (const [provider, stats] of Object.entries(summary.providers)) {
            console.log(`\n${provider.toUpperCase()} (유료 API):`);
        ...
        console.log('\n' + '='.repeat(60));
        console.log(`💰 총 API 비용: $${summary.totalCost} (₩${summary.totalCostKRW})`);
        console.log('='.repeat(60));
